...
def extractData(folder, index):
  ...
  for videoName in videoFileNames:
    try:
      ...
      #i = 0
      for _ in range(1):
        cap.set(cv2.CAP_PROP_POS_FRAMES, (frames * random.random()) % (frames-1))
        ret, frame = cap.read()
...
def pickleData(folder):
  ...
  pickle_file = os.path.join(folder, 'actionProc.pickle')
  try:
